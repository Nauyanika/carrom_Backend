...
}, 60*60*1000); // for ik hour

//24*60*60*1000);     for one day
...
mainOrderId=user1[0].orderid
    ...
    const  responseData = await conn.query(sql, req.body.customer_name);
      if (responseData.length > 0) {
        let stokezPointId = responseData[0].id;
        const tpoints = parseInt(req.body.order_amount) + parseInt(responseData[0].point);
        console.log(tpoints,"amount");

        sql = "UPDATE users SET ? WHERE username =? ";

        const Amount = await conn.query(sql,  [{ point: tpoints },req.body.customer_name]);
        if(Amount.length > 0){
        statusCode = 201;
            message = "update data ";
        ...
        console.log(Amount,"amount");
          ...
          statusCode = 403;
          message ="Data is not valid in database";
        }
    } }
     catch (error) {
      console.log("error------",error)
  ...
  app.post('/getfetchImage', async (req, res) => {
    ...
    try {
     ...
     //   isCardAdded=true;
        ...
        image.avatarimage = "http://localhost/New Carrom-backend/images/" + file;
        data1.image = image;
        data = data1;
      } else {
        statusCode = 404;
...
const agent=conn.query(sql,req.body.id)

if(agent.length>0){

  sql="update users set username =? where id=?"
const resp=conn.query(sql,req.body.username,id)
